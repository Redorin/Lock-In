...
<head>
    ...
    <title>CampuSIMS — Forgot Password</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root { --bg:#091510;--glass:rgba(255,255,255,.04);--glass-border:rgba(255,255,255,.08);--accent:#00e5a0;--text:#ddeee6;--text-soft:rgba(221,238,230,.55);--text-muted:rgba(221,238,230,.28);--radius-lg:22px;--radius-md:14px;--radius-sm:10px;--danger:#ff7070; }
        ...
        .auth-sub { font-size:.85rem;color:var(--text-soft);margin-bottom:28px;line-height:1.6; }
        ...
        .btn-submit { width:100%;padding:13px;background:var(--accent);color:#091510;font-family:'Outfit',sans-serif;font-weight:700;font-size:1rem;border:none;border-radius:var(--radius-md);cursor:pointer;box-shadow:0 4px 24px rgba(0,229,160,.25);transition:opacity .18s,transform .15s; }
        ...
        .reset-link-box { background:rgba(0,229,160,.06);border:1px solid rgba(0,229,160,.2);border-radius:var(--radius-md);padding:16px;margin-bottom:20px; }
        .reset-link-box p { font-size:.8rem;color:var(--text-soft);margin-bottom:8px; }
        .reset-link-box a { word-break:break-all;font-size:.78rem;color:var(--accent); }
        .copy-btn { display:inline-block;margin-top:10px;padding:6px 14px;background:rgba(0,229,160,.12);border:1px solid rgba(0,229,160,.2);color:var(--accent);border-radius:8px;font-size:.75rem;font-weight:600;cursor:pointer;font-family:'Outfit',sans-serif; }
    </style>
</head>
<body>
<div class="page">
    <div class="auth-wrap">
        ...
        <div class="auth-card">
            ...
            <p class="auth-sub">Enter your registered email and a reset link will be generated for you.</p>

            @if(session('reset_link'))
                <div class="reset-link-box">
                    <p>✅ Reset link generated. Copy and open it in your browser:</p>
                    <a href="{{ session('reset_link') }}" id="resetLink">{{ session('reset_link') }}</a>
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="field">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="you@school.edu" required>
                </div>
...
<script>
function copyLink() {
    navigator.clipboard.writeText(document.getElementById('resetLink').href).then(() => {
